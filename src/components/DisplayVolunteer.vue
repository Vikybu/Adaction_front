<template>
  <div class="flex flex-col items-center w-full mt-6 max-w-md mx-auto">

    <!-- Bouton d'ajout -->
    <button
      @click="goToCreateVolunteer"
      class="flex items-center justify-center gap-2 bg-emerald-600 text-white font-medium rounded-lg py-3 w-[90%] max-w-md shadow hover:bg-emerald-700 transition"
    >
      <svg
    ...
    <div class="flex flex-wrap justify-center items-center gap-3 mt-4 w-full">
      <input
        v-model="searchInput"
        type="text"
        placeholder="Rechercher un·e bénévole"
        class="w-[40%] min-w-[200px] border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-600"
      />
      <select
    ...
    <!-- Liste des bénévoles -->
    <div v-if="volunteers.length > 0" class="mt-6 w-full flex flex-col items-center gap-4">
      <div
        v-for="volunteer in volunteers"
        :key="volunteer.id"
        class="flex justify-between items-center border border-gray-300 rounded-lg w-[85%] max-w-2xl p-4 bg-white shadow-sm hover:shadow-md transition"
      >
        <div class="flex flex-wrap items-center gap-4">
          <p class="font-semibold text-gray-800">{{ volunteer.firstName }}</p>
    ...
    <!-- Aucun bénévole -->
    <p v-else class="text-gray-500 text-sm mt-6 italic">Aucun·e bénévole trouvé·e</p>
